...
void HeadTracker::startTracking(UIInterfaceOrientation orientation) //运行位置跟踪
{
    ...
    NSOperationQueue *accelerometerQueue = [[NSOperationQueue alloc] init]; //加速度队列
    NSOperationQueue *gyroQueue = [[NSOperationQueue alloc] init];  ///陀螺仪队列
...
GLKMatrix4 HeadTracker::lastHeadView()
{
    ...
    if (_neckModelEnabled) ///判断脖子是否可以扭动
    {
